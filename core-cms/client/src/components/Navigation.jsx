    ...
    <nav className="fixed w-full z-50 top-0 bg-black/80 backdrop-blur-md border-b border-white/5">
      <div className="max-w-7xl mx-auto px-6 py-6 flex justify-between items-center">
        <Link to="/" className="flex items-center">
          <span className="text-2xl font-black text-white tracking-tight">CORE</span>
        </Link>
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          <div className="relative">
            <button 
              className="nav-link text-sm font-medium text-white flex items-center gap-1"
            ...
            {solutionsOpen && (
              <div 
                className="absolute top-full left-0 mt-2 w-56 bg-core-gray border border-white/10 rounded-xl overflow-hidden shadow-2xl"
                onMouseEnter={() => setSolutionsOpen(true)}
                onMouseLeave={() => setSolutionsOpen(false)}
              >
                <Link 
